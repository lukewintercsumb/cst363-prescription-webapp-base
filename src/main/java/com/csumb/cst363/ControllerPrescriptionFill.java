	...
	public String processFillForm(Prescription p, Model model) {
		...
		PreparedStatement ps = con.prepareStatement("SELECT pharmacy_id FROM pharmacy WHERE pharmacy_name = ? AND address = ?");
			...
			if (rs.next()) {
				...
				System.out.println(p.getPharmacyName());
				System.out.println(p.getPharmacyAddress());
...
//		rs = ps.executeQuery();
		...
		ps = con.prepareStatement("SELECT * FROM prescription WHERE rx_id = ? AND prescrption_patient_id = (SELECT patient_id FROM patient WHERE last_name = ?)");
		ps.setString(1, p.getRxid());
		ps.setString(2, p.getPatientLastName());
		rs = ps.executeQuery();
...
//	ps.executeUpdate();
	...
	ps = con.prepareStatement("UPDATE prescription SET pharmacyID = ?, dateFilled = ? WHERE rx_id = ?");
	ps.setInt(1, p.getPharmacyID());
	ps.setString(2, LocalDate.now().toString());
	ps.setString(3, p.getRxid());
	ps.executeUpdate();
